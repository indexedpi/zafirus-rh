      ...
      <div class="flex-1 overflow-y-auto scrollbar-hide">
        @if (sortedCases().length === 0) {
          <div class="flex flex-col items-center justify-center h-full text-center px-6 py-12">
            ...
            <p class="text-sm font-semibold text-[var(--text-secondary)] mb-1">No se encontraron casos de onboarding</p>
            <p class="text-xs text-[var(--text-tertiary)] mb-4">Crea un nuevo caso para comenzar el onboarding operativo.</p>
            <button
              (click)="modalOpen.set(true)"
          ...
          <div class="py-1 space-y-1 px-1.5">
            @for (c of sortedCases(); track c.id) {
              <button
                ...
                [class.bg-brand-subtle]="c.id === svc.selectedCaseId()"
                [class.border-transparent]="c.id !== svc.selectedCaseId()"
                [style.backgroundColor]="c.id === svc.selectedCaseId() ? 'var(--brand-primary-subtle)' : ''"
                [style.borderColor]="c.id === svc.selectedCaseId() ? 'rgba(69,156,219,0.2)' : ''"
              >
                <div class="flex items-center justify-between w-full gap-2">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    @if (c.id === svc.selectedCaseId()) {
                      <span class="w-1.5 h-1.5 rounded-full bg-[var(--brand-primary)] animate-pulse flex-shrink-0"></span>
                    }
                  </div>
                  <span
                ...
                <div class="flex items-center justify-between gap-2 border-t border-[var(--border-subtle)]/60 pt-2 mt-2">
                  <span class="text-[10px] font-semibold text-[var(--text-secondary)] truncate flex items-center gap-1 leading-none">
                    <svg class="w-2.5 h-2.5 text-[var(--brand-primary)] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
