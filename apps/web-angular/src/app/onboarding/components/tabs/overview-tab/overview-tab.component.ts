...
const ACTION_LABELS: Record<string, string> = {
  ...
  email_approved: 'Plantilla de email aprobada', candidate_data_consolidated: 'Datos consolidados',
};
...
@Component({
  ...
  template: `
    @if (svc.selectedCase(); as c) {
      <div class="space-y-6">
        <!-- Identity header -->
        <div class="flex items-start gap-3 lg:gap-4 pb-2">
          <div class="w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0" style="background: var(--brand-primary)">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2.5 mt-1 text-xs text-[var(--text-secondary)] flex-wrap">
              <span>{{ c.employee.role }}</span>
              <span class="text-[var(--text-tertiary)]">·</span>
              <span class="uppercase font-mono">{{ teamLabel(c.employee.team) }}</span>
            </div>
            <div class="flex items-center gap-3 mt-2 text-[11px] text-[var(--text-tertiary)] flex-wrap">
              <span>{{ c.employee.cityId }}, {{ countryName(c.employee.countryId) }}</span>
              <span class="text-[var(--text-tertiary)]">·</span>
              <span>Inicio: {{ formatDate(c.employee.startDate) }}</span>
              <span class="text-[var(--text-tertiary)]">·</span>
              <span class="text-[var(--brand-primary)] font-bold uppercase tracking-wider">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
          <!-- Journey -->
          <section class="flex-1 min-w-0 bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-subtle)] p-6">
            ...
            <ol class="relative">
              @for (stage of journeyStages; track stage.id; let i = $index; let last = $last) {
                <li class="relative flex items-start pb-6 last:pb-0">
                  @if (!last) {
                    <span class="absolute left-3 top-6 bottom-0 w-px -ml-px"
                      [class]="i < currentIndex(c.status, c.auditLog) || (c.status === 'operative' && i <= currentIndex(c.status, c.auditLog)) ? 'bg-[var(--brand-primary)]' : 'bg-[var(--border-subtle)]'"
                    ></span>
                  }
                  <div class="relative flex items-center justify-center w-6 h-6 rounded-full bg-[var(--bg-elevated)] z-10 flex-shrink-0 mr-3">
                    @if (i < currentIndex(c.status, c.auditLog) || (c.status === 'operative' && last)) {
                      <svg class="w-5 h-5 text-[var(--brand-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    } @else if (i === currentIndex(c.status, c.auditLog) && c.status !== 'operative') {
                      <span class="w-3 h-3 rounded-full bg-[var(--brand-primary)] ring-4 ring-[var(--brand-primary-subtle)]"></span>
                    } @else {
                      <span class="w-2.5 h-2.5 rounded-full border-2 border-[var(--border-default)]"></span>
                    }
                  </div>
                  <div class="flex-1 min-w-0 pt-0.5">
                    <p class="text-sm"
                      [class]="i === currentIndex(c.status, c.auditLog) && c.status !== 'operative' ? 'text-[var(--brand-primary)] font-bold'
                    ...
                    @if (i === currentIndex(c.status, c.auditLog) && c.status !== 'operative') {
                      <p class="text-[11px] uppercase tracking-wider font-semibold text-[var(--brand-primary)] mt-1">Etapa actual</p>
                    }
                  </div>
                </li>
              }
          ...
          <!-- Right: next + recent -->
          <aside class="w-full lg:w-80 flex-shrink-0 space-y-6 min-w-0">
            <div class="bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-subtle)] p-6">
              <h3 class="text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider mb-3">Siguiente hito</h3>
              ...
              @if (c.status === 'operative') {
                <p class="text-xs text-[var(--status-success)] mt-2 font-medium">El alta operativa fue completada.</p>
              }
            </div>
            ...
            <div class="bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-subtle)] p-6">
              <div class="flex items-baseline justify-between mb-4 gap-3 flex-wrap">
                ...
                <button (click)="navigateTo.emit('audit')" class="text-[10px] font-semibold text-[var(--brand-primary)] uppercase tracking-wider hover:underline">
                  ...
                  @for (ev of recentEvents(c.auditLog); track ev.id; let i = $index) {
                    <li class="relative pb-4 last:pb-0">
                      @if (i < recentEvents(c.auditLog).length - 1) {
                        <span class="absolute bg-[var(--border-subtle)]" style="left: -13px; top: 13px; bottom: -4px; width: 1px"></span>
                      }
                      <span class="absolute z-10 rounded-full border-2 border-[var(--border-default)] bg-[var(--bg-base)]" style="left: -17px; top: 4px; width: 8px; height: 8px"></span>
...
export class OverviewTabComponent {
  ...
  statusColor(s: CaseStatus): string {
    const m: Record<string, string> = { draft: 'var(--text-secondary)', blocked: 'var(--status-error)', operative: 'var(--status-success)', cancelled: 'var(--text-tertiary)', hr_review: 'var(--status-warning)', active_pending_automation: 'var(--status-warning)' };
    return m[s] || 'var(--status-info)';
  }
  statusBg(s: CaseStatus): string {
    const m: Record<string, string> = { operative: 'var(--status-success-subtle)', blocked: 'var(--status-error-subtle)', hr_review: 'var(--status-warning-subtle)', active_pending_automation: 'var(--status-warning-subtle)' };
  ...
  nextStep(status: CaseStatus, consolidated: boolean): string {
    const m: Record<string, string> = {
      ...
      candidate_submitted: 'Paso: Iniciar revisión', ready_to_activate: 'Paso: Activar Workspace',
      active_pending_automation: 'Paso: Tareas en curso', operative: 'Paso: Colaborador Operativo',
      blocked: 'Paso: Resolver bloqueo', cancelled: 'Paso: Caso archivado',
    };
    if (status === 'hr_review') return consolidated ? 'Paso: Aprobar onboarding' : 'Paso: Consolidar datos';
    return m[status] || '';
  }
  ...
  milestone(status: CaseStatus, hasFailed: boolean): string {
    const m: Record<string, string> = {
      ...
      ready_to_activate: 'Activar onboarding', operative: 'Caso operativo',
